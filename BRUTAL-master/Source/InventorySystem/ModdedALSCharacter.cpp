...
void AModdedALSCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("LeftMouseClick", this, &AModdedALSCharacter::SwingLeftArm);
	PlayerInputComponent->BindAxis("RightMouseClick", this, &AModdedALSCharacter::SwingRightArm);
...
void AModdedALSCharacter::SwingLeftArm(float Input)
{
	// Trigger animation or logic for left arm swinging
	ApplyForceToArm(Input);
}

void AModdedALSCharacter::SwingRightArm(float Input)
{
	// Trigger animation or logic for right arm swinging

	ApplyForceToArm(Input);
}

void AModdedALSCharacter::PlayArmSwingAnimation()
{
	FVector2D MousePosition;
	if (APlayerController* PC = Cast<APlayerController>(Controller))
	{
		//FVector MouseVect = PC->GetMousePosition(MousePosition.X, MousePosition.Y);
...
// Example function to apply force to a specific bone in the skeletal mesh
void AModdedALSCharacter::ApplyForceToArm(float Input)
{
	FVector Force = { 1000,0,0 };
	// Get the skeletal mesh of the character
	USkeletalMeshComponent* SkeletalMesha = GetMesh();
	if (!SkeletalMesha)
	{
		return;  // Return if no skeletal mesh is found
	}

	// Determine the bone name based on the arm (left or right)
	// Make sure you have the correct bone names
	FName ArmBoneName = 1 ? TEXT("lowerarm_l") : TEXT("lowerarm_r");
	UE_LOG(LogTemp, Warning, TEXT("void AModdedALSCharacter::ApplyForceToArm , FVector Force = %s"), Force);
	// Get the location of the specified bone in world space
	FVector BoneLocation = SkeletalMesha->GetBoneLocation(ArmBoneName);

	// Apply force to the skeletal mesh at the bone's location
	SkeletalMesha->AddForceAtLocation(Force, BoneLocation, ArmBoneName);
}

void AModdedALSCharacter::EnableRagdoll()
{
	// Get the skeletal mesh component
	USkeletalMeshComponent* MeshComp = GetMesh();
	if (MeshComp)
	{
		// You can specify which bones to become ragdolls
		MeshComp->SetAllBodiesBelowSimulatePhysics("pelvis", true, true);
		MeshComp->SetAllBodiesBelowPhysicsBlendWeight("pelvis", 0.5f, true, true);
	}
}